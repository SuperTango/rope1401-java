program AUTOCODER
  ...
  use MACRO_PASS_M, only: ADD_PATH, MACRO_PASS, MA_MACRO
  ...
  character(len=*), parameter :: &
    & Version = &
    & '1401 Autocoder (c) Van Snyder <van.snyder@sbcglobal.net> 2009 version 3.6'
  ...
  do
    ...
    if ( arg(2:3) == 'a ' ) then
      ...
      print *, '                      IBM A  & |  #  @  %  ~  {  "   }'
      print *, '                      IBM H  + |  =  ''  (  ~  {  "   }'
      print *, '          -I => A path to search for macros.  First is always "."'
      ...
      print *, '          -M => MA is a macro in file ma'
      print *, '          -o[ ]file => Object "deck" file'
      print *, '          -p[ ]# => Page length in lines'
