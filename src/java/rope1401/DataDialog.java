...
{
    Border border1 =
        new TitledBorder(BorderFactory.createLineBorder(Color.white, 2),
                         "Card read/punch");
    ...
    private String browseAction(String filePath, JTextField textField,
                                RopeFileFilter filter)
    {
    ...
    private boolean handleUnit(String unitName, JCheckBox checkBox, JTextField filePath)
    {
        if (checkBox.isSelected()) {
            ...
            if (path.length() > 0) {
                ...
                messageText.setText("*** Unit " + unitName +
                                    ": Uncheck the checkbox or enter a file path.");
                return false;
            }
        }
        else {
